...
function loadDarkMode() {
  ...
  if (isDark) {
    ...
    btnDark.textContent = '☀️ Light mode';
  }
}
...
async function render() {
  ...
  filtered.forEach(task => {
    ...
    div.className = 'flex items-center justify-between bg-white dark:bg-gray-800 rounded-xl shadow px-5 py-4 transition-all';
    div.innerHTML = `
      <div class="flex items-center gap-3">
        <input type="checkbox" ${task.completed ? 'checked' : ''}
          class="w-5 h-5 accent-cyan-500 cursor-pointer"
          data-id="${task.id}" />
        <span class="text-gray-800 dark:text-white font-medium ${task.completed ? 'line-through text-gray-400' : ''}">
          ${task.title}
        </span>
      </div>
      <button data-id="${task.id}"
        class="btn-delete text-red-400 hover:text-red-600 transition text-xl">
...
function updateFilterButtons() {
  filterBtns.forEach(btn => {
    ...
    btn.className = `filter-btn flex-1 py-2 rounded-lg text-sm font-semibold transition ${
      isActive
        ? 'bg-blue-900 dark:bg-cyan-600 text-white'
